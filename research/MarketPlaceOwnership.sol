...
contract MarketplaceOwnerShip is ERC1056 {
...
entityOwner(address identity) public returns (address) {
    return entityOwners[identity]; 
}

//add
_addEntity(bytes32 name, address identity, address owner, bytes32 entityType) internal onlyValidUser{
    require(validDelegate(entityOwner(identity),msg.sender, DELEGATE_ADMIN))
